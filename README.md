...
Projeto da AV2 para a parte **Dev B**, com frontend web em HTML, CSS e JavaScript e automação RPA para envio de notificações por WhatsApp e Gmail. A documentação da atividade atribui ao Dev B a responsabilidade por frontend, mensageria e automação com PyWhatKit e SMTP Gmail, enquanto os critérios de avaliação exigem consumo real da API, filtros por múltiplos critérios e envio comprovado das mensagens.
...
Este projeto entrega a base do portal de vagas e das automações que fazem parte da sua responsabilidade na AV2. O frontend foi preparado para ser publicado no GitHub Pages e consumir o backend do colega via ngrok, enquanto os scripts Python ficam prontos para execução local na apresentação, já que o GitHub Pages não executa código Python.

## Frontend

O frontend atende ao requisito de interface funcional com consumo real da API e filtros de vagas por pelo menos dois critérios. A página já inclui:
...
A atividade aceita entrega por link público do GitHub, então este frontend pode ser hospedado diretamente no GitHub Pages.
...
Como o projeto é estático, você pode abrir o `index.html` diretamente no navegador. Mesmo assim, para evitar limitações locais, o ideal é usar um servidor simples.
...
Os scripts de automação foram montados para cumprir o requisito de envio real via WhatsApp Web e Gmail, com tratamento de erros e registro de logs.
...
O arquivo `logger_utils.py` cria automaticamente a pasta `logs/` e grava os arquivos de log da automação. Isso ajuda a demonstrar sucesso ou falha dos envios, algo explicitamente cobrado na avaliação.

## WhatsApp

O script `enviar_whatsapp.py` usa `PyWhatKit` para abrir o WhatsApp Web e disparar uma mensagem para um número válido, como exigido pela atividade.
...
O script `enviar_email.py` usa `smtplib` com o servidor SMTP do Gmail, em linha com a definição de Dev B para mensageria por e-mail.
...
A própria atividade pede demonstração em tempo real do portal e prova de envio real de WhatsApp e e-mail para um contato válido da turma-alvo. Por isso, a melhor estratégia é:

- publicar o frontend no GitHub Pages;
- deixar o backend do colega ativo no ngrok;
- testar CORS e endpoints antes da apresentação;
- deixar WhatsApp Web já autenticado;
...
Com base nos requisitos da AV2, esta base ainda pode evoluir para:

- formulário real de candidatura integrado ao backend;
- chatbot com fluxo saudação, filtro e candidatura;
